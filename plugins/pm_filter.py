...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⚡ Ŧ๏ɭɭ๏ฬ ⚡', url='https://instagram.com/broken_.editzz._?igshid=YzdkMWQ2MWU='),
            InlineKeyboardButton('🤖 UᎮDΛTΞS 🤖', url='https://t.me/tamilblasterslinkz')
        ], [
        ...
        buttons = [[
            InlineKeyboardButton('👩‍🦯 𝕭𝖆𝖈𝖐', callback_data='manuelfilter')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('👩‍🦯 𝕭𝖆𝖈𝖐', callback_data='extra')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", 
                    url=await (f"https://t.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
            ]
            for file in files
        ]
